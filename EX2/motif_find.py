...
def forward(seq, emissions, tau, run_log=True):
    ...
    for col in range(1, f.shape[1]):
        if run_log:
            ...
            for row in range(f.shape[0]):
                f[row, col] = np.dot(f[:, col - 1], tau[:, row]) * emissions[seq[col]][row]

    return f


def backward(seq, emissions, tau, run_log=True):
    ...
    for col in range(b.shape[1] - 2, -1, -1):
        if run_log:
            with np.errstate(divide='ignore'):
                temp = logdot(b[:, col + 1], tau.T)
            b[:, col] = temp + emissions[seq[col]]
        else:
            for row in range(b.shape[0]):
                b[row, col] = np.sum(b[:, col + 1] * tau[row, :].T * emissions[seq[col]][row])

    return b


def posterior(seq, emissions, tau, run_log=True):
    f = forward(seq, emissions, tau, run_log)
    b = backward(seq, emissions, tau, run_log)
...
def main():
    ...
    if args.alg == 'viterbi':
        ...
        vec = forward(seq, emissions, tau, args.log)[:, -1]
        print(logsumexp(vec) if args.log else np.log(np.sum(vec)))
    elif args.alg == 'backward':
        vec = backward(seq, emissions, tau, args.log)[:, 0]
        print(logsumexp(vec) if args.log else np.log(np.sum(vec)))
    elif args.alg == 'posterior':
        pretty_print(*posterior(seq, emissions, tau, args.log))
